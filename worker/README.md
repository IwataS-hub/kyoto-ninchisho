...
- **レート制限（重要）**: `gemini-3.7-flash` の**無料枠は1日20リクエスト**でした
  （2026-08-19 に実測。エラー本文の
  `generate_content_free_tier_requests, limit: 20, model: gemini-3.7-flash` で確認）。
  相談1件で最大8リクエストを使うため、**無料枠のままでは1日2〜3件で上限に達します**。
  実運用には課金の有効化が必須です（2026-08-19 に有効化済み）。
  ドキュメントに固定の表は無く、プロジェクトごとの現在値は
  [AI Studio のレート制限画面](https://aistudio.google.com/rate-limit)で確認します。
  429（quota exceeded）が出る場合はWorkerがエラー本文の要点（超過したクォータ名・上限値・
  リトライ推奨秒数）をフロントまで返すため、画面の「詳細:」表示で判別できます。
- **503 の断続的な発生**: `gemini-3.7-flash` は
  「This model is currently experiencing high demand」の503を断続的に返すことがあります
  （2026-08-19 の実測では多くのリクエストが初回503）。フロントには「再試行」ボタンがあり、
  評価スクリプトは段階的バックオフで再試行します。
